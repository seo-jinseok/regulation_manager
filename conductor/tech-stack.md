# Technology Stack

## Programming Language
*   **Python**: The core language for the application, chosen for its strong ecosystem in text processing and AI integration.

## Core Frameworks & Libraries
*   **LlamaIndex**: Used for managing interactions with Large Language Models (LLMs), enabling the hybrid preprocessing engine.
*   **BeautifulSoup4**: Employed for parsing HTML content generated from HWP files, allowing for precise element extraction.
*   **Pandas**: Utilized for internal data manipulation and potential intermediate processing steps.
*   **markdownify**: Converts HTML content into Markdown format, preserving document structure.
*   **pyhwp**: A library for reading and processing Hangul Word Processor (HWP) files.
*   **questionary**: Provides interactive command-line prompts for a better user experience.
*   **rich**: Enables rich text formatting and pretty printing in the terminal output.
*   **python-dotenv**: Manages environment variables and configuration secrets.

## Database
*   **File-based JSON**: The system outputs structured data directly to JSON files, serving as a flat-file database.

