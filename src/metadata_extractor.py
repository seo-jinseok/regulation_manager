...
class MetadataExtractor:
    ...
    def extract(self, text: str) -> Dict[str, Any]:
        ...
        toc_map = {
            self._normalize_title(item["title"]): item["rule_code"] for item in toc_list
        }
    ...
    def _extract_toc(self, text: str) -> List[Dict[str, str]]:
        ...
        entry_pattern = re.compile(
            r"^\s*(?P<title>.*?)\s+(?P<code>\d+[-—]\d+[-—]\d+)(?:[~～]\d+)?\s*$"
        )
    ...
    def _extract_index_alpha(
        self, text: str, toc_map: Dict[str, str] = None
    ) -> List[Dict[str, str]]:
        # Look for "찾아보기" -> "<가나다순>"
    ...
    def _extract_index_dept(
        ...
        end_patterns = [
            re.compile(
                r"^[\|\s]*제\s*\d+\s*편"
            ),  # part header with optional table chars
        ...
        for line in lines:
            ...
            if m:
                ...
                code = (
                    toc_map.get(lookup_key, self._normalize_code(raw_code))
                    if toc_map
                    else self._normalize_code(raw_code)
                )

                dept_index[current_dept].append({"title": title, "rule_code": code})
            else:
                if "학교법인" in line and not re.search(
    ...
    def _parse_entries(
        ...
        for line in lines:
            ...
            if m:
                ...
                entries.append({"title": title, "rule_code": code})
        return entries
