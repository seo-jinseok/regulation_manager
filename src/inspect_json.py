...
def analyze_json(path):
    ...
    for i, doc in enumerate(docs):
        ...
        if articles:
            print(f"First Article: {articles[0].get('display_no')} - {articles[0].get('title')}")
            print(f"Last Article: {articles[-1].get('display_no')} - {articles[-1].get('title')}")
...
def main():
    ...
    parser.add_argument("input_path", nargs="?", help="Path to a JSON file (default: first in data/output)")
