...
class RegulationFormatter:
    ...
    def parse(
        ...
        if html_content:
            ...
            if verbose_callback and header_entries:
                verbose_callback(
                    f"[dim]  - 헤더 메타데이터 {len(header_entries)}개 발견[/dim]"
                )
        ...
        for doc_data in flat_doc_data:
            ...
            # Appendices Parsing
            addenda, attached_files = self._parse_appendices(
                doc_data.get("appendices", []), html_content=html_content
            )
            ...
            if header_entries:
                ...
                relevant = [
                    h for h in header_entries if self._titles_match(title, h["prefix"])
                ]
                ...
                if relevant:
                    ...
                    same_code_headers = [
                        h for h in header_entries if h["rule_code"] == rule_code
                    ]
            ...
            if verbose_callback:
                ...
                verbose_callback(
                    f"[dim]  - 분석 완료: '{title_display}' ({art_count}개 조항, {add_count}개 부칙, {att_count}개 별표/서식)[/dim]"
                )
    ...
    def _doc_has_content(self, doc: Dict[str, Any]) -> bool:
        return (
            bool(doc.get("content"))
            or bool(doc.get("addenda"))
            or bool(doc.get("attached_files"))
        )
    ...
    def _merge_adjacent_docs(self, docs: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for doc in docs[1:]:
            ...
            if (
                not doc.get("title")
                and doc.get("content")
                and current.get("part") == doc.get("part")
            ):
    ...
    def _populate_index_docs(
        self, docs: List[Dict[str, Any]], extracted_metadata: Dict[str, Any]
    ) -> None:
        ...
        for doc in docs:
            ...
            if kind == "toc" and toc_entries:
                ...
                doc["part"] = None
                dept_nodes = []
                for dept_idx, (dept, entries) in enumerate(
                    sorted(index_dept_entries.items()), 1
                ):
    ...
    def _parse_appendices(
        ...
        while idx < len(segments):
            ...
            if "부칙" in header_norm:
                ...
                add_refs = (
                    self._extract_references(content) if not children_nodes else []
                )
                ...
                attached_files.append({"title": header, "text": content})
        ...
        for div in header_areas:
            ...
            if match:
                ...
                metadata_list.append(
                    {"rule_code": rule_code, "page": page_number, "prefix": prefix}
                )
    ...
    def _titles_match(self, doc_title: str, header_prefix: str) -> bool:
        ...
        return (
            norm_doc == norm_header
            or norm_doc in norm_header
            or norm_header in norm_doc
        )
    ...
    def _parse_addenda_text(self, text: str) -> List[Dict]:
        ...
        for line in lines:
            ...
            if current_node:
                ...
                nodes.append(
                    self._create_node("text", "", None, line, confidence_score=0.5)
                )
                current_node = nodes[-1]
