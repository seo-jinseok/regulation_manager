...
class InteractiveWizard:
    ...
    def run(self) -> Any:
        ...
        if not target_dir or not target_dir.exists():
            ...
            if questionary.confirm(
                f"    지금 '{target_dir}' 폴더를 생성하시겠습니까?"
            ).ask():
                target_dir.mkdir(parents=True, exist_ok=True)
                console.print(
                    "    [green]>> 생성 완료.[/green] 파일을 이동시킨 후 다시 실행해주세요."
                )
            sys.exit(0)
        ...
        console.print(
            Panel(
                "[bold green]설정 완료![/bold green] 처리를 시작합니다...",
                style="green",
            )
        )
        return config
    ...
    def _select_file_or_folder(self, files: List[Path], base_dir: Path) -> Path:
        ...
        table.add_row(
            "0",
            "",
            "[bold yellow]전체 일괄 처리[/bold yellow]",
            str(base_dir.relative_to(self.root_dir)),
        )
        ...
        choices = []
        choices.append(
            Choice(title="0. 전체 일괄 처리 (All)", value=base_dir)
        )  # Return base dir for all
    ...
    def _configure_options(self) -> Any:
        ...
        config.use_llm = questionary.confirm(
            "LLM(AI)을 사용하여 텍스트 품질을 보정하시겠습니까?", default=False
        ).ask()
        ...
        config.verbose = questionary.confirm(
            "상세 로그를 보시겠습니까? (디버깅용)", default=False
        ).ask()
        ...
        if config.use_llm:
            ...
            config.provider = questionary.select(
                "LLM 제공자를 선택하세요:", choices=providers, default=default_provider
            ).ask()
        ...
        # Force Re-conversion
        config.force = questionary.confirm(
            "기존 캐시 무시 (강제 변환)?", default=False
        ).ask()
