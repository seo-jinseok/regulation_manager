...
def inspect_hierarchy(input_dir: str):
    ...
    for i, node in enumerate(path):
        indent = "  " * i
        print(
            f"{indent}- {node['type']} {node.get('display_no', '')} {node.get('title') or ''} | {node.get('text', '')[:50]}"
        )


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Inspect hierarchy depth for a sample doc."
    )
    parser.add_argument(
        "--input_dir", default="data/output", help="Directory containing JSON outputs."
    )
