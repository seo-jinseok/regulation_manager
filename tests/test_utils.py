...
class TestUtils(unittest.TestCase):
    def test_verify_json_logic(self):
        # 1. Valid case
        valid_data = {"docs": [{"title": "Test", "content": [{"type": "article", "text": "Valid"}]}]}
        # 2. Invalid cases
        ...
        try:
            with open("tmp_test_verify/valid.json", "w") as f: json.dump(valid_data, f)
            with open("tmp_test_verify/invalid.json", "w") as f: json.dump(invalid_data, f)
            with open("tmp_test_verify/leak.json", "w") as f: json.dump(leak_data, f)
