...
class TestMainFlow(unittest.TestCase):
    ...
    def test_main_full_flow(self, MockReaderClass, MockLLM, MockFormatter, MockPre):
        ...
        MockFormatter.return_value.parse.return_value = [{"title": "Test Doc", "content": [], "metadata": {}}]
        
        # Patch sys.argv and call main
        with patch('sys.argv', ['main.py', str(self.input_file), '--output_dir', str(self.output_dir), '--force']):
            try:
                main()
            except SystemExit:
                pass
