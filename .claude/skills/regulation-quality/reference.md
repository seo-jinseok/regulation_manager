# Ragulation Quality - Reference Guide

 knackExternal resources, related work, and references for Regulation Manager Q&A quality evaluation.
...
metric = AnswerRelevancyMetric(threshold=0.0)
...
### CLI Integration
```bash
# Regulation Manager query execution
regulation ask "{query}" --format json

# Evaluation run
python -m ragulation_quality --full

# Targeted evaluation
python -m ragulation_quality --persona professor --category edge_cases
```
...
3. Execute Regulation Manager CLI -> regulation ask "{query}"
...
| Accuracy | 1.85 | No hallucinations |
| Completeness | 0.75 | All key info present |
| Citations | 1.70 | Accurate regulation refs |
| Context Relevance | 1.75 | Relevant sources |
...
| Graduate | 1.83 | 82% |
| Staff | 1.81 | 80% |
| Undergraduate | 1.79 | 75% |
| Parent | 1.78 | 73% |
| International | 1.76 | 70% |
