# Ragulation Quality Evaluation Module

LLM-as-Judge evaluation system for Regulation Manager Q&A quality assessment.

## Evaluation Pipeline

### Step 1: Query Execution

Execute queries through the Regulation Manager CLI:
...
```python
judge_prompt = f"""
You are an expert evaluator for Regulation Manager Q&A systems.
Evaluate the quality of the following response based on the criteria below.
...
def check_citation_quality(answer: str) -> float:
    ...
    # Perfect: "규정명 + 제X조"
    if re.search(r'\w+규[정|程]\s*제\d+조', answer):
        return 1.0
